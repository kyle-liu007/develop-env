...
def parse_cmd_file(root_dir, cmdfile_path):
...
def gen_compile_commands(cmd_file_search_path, root_dir):
    print("Building *.o.cmd file list...", file=sys.stderr)

    root_dir = os.path.abspath(root_dir)
    print("cmd_file_search_path:", cmd_file_search_path);
    print("out_dir", root_dir);

    if not cmd_file_search_path:
        cmd_file_search_path = [root_dir]
    ...
    try:
        for compdb_chunk in pool.imap_unordered(functools.partial(parse_cmd_file, root_dir), cmd_files, chunksize=int(math.sqrt(len(cmd_files)))):
...
def main():
    ...
    cmd_parser.add_argument('-r', '--root-dir', type=str, default=os.getcwd(), help="Building system root directory")
